const String stcFontStr = 'STC';

const String baseUrl = 'https://menu-demo.technolanes.com/menu';

const int HOME_INDEX= 0;
const int SECTIONS_INDEX= 1;
const int ADDSECTIONS_INDEX= 2;
const int EDITSECTIONS_INDEX= 3;
const int PRODUCTS_INDEX = 4;
const int ADDPRODUCT_INDEX= 5;
const int EDITPRODUCT_INDEX= 6;
const int EMPLOYEES_INDEX = 10;
const int ADDEMPLOYEE_INDEX = 11;
const int EDITEMPLOYEE_INDEX = 12;
const int SIZES_INDEX = 13;
const int ADDSIZE_INDEX = 14;
const int EDITSIZE_INDEX = 15;
const int ADDMAINSIZE_INDEX = 16;
const int ORDERSHISTORY_INDEX = 17;