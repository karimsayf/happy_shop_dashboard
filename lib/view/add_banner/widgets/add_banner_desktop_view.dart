...
class AddBannerDesktopView extends StatefulWidget {
  const AddBannerDesktopView({super.key});

  @override
  State<AddBannerDesktopView> createState() => _AddBannerDesktopViewState();
}

class _AddBannerDesktopViewState extends State<AddBannerDesktopView> {
  late final addBannerViewModel = Provider.of<AddBannerViewModel>(context);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Form(
        key: addBannerViewModel.formKey,
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                },child: const Text('البنرات',style: TextStyle(color: AppColors.mainColor,fontSize: 13),)),
                const SizedBox(width: 10,),
                const Text('/',style: TextStyle(color: AppColors.mainColor,fontSize: 13),),
                const SizedBox(width: 10,),
                const Text('إضافة بانر',style: TextStyle(color: Colors.grey,fontSize: 13),),
              ],
            ),
            _buildDivider(),
            const Text(
            ...
            _buildSection(
              title: 'بيانات البانر',
              desc: 'قم برفع تفاصيل البانر',
              children: [
                UploadFileWidget(
                  title: "تحميل صورة",
                  file: addBannerViewModel.file,
                  setFileOnProvider: (file) {
                    setState(() {
                      addBannerViewModel.file = file;
                    });
                  },
                ),
              ],
            ...
            Row(
              ...
              children: [
                InkWell(
                  onTap: addBannerViewModel.loading ? (){} :  () async{
                    await Provider.of<AddBannerViewModel>(context,listen: false).addBanner(context);
                  },
                  ...
                  child: Container(
                    ...
                    height: 40,
                    width: 120,
                    child: addBannerViewModel.loading? const Center(
