...
class AddCityDesktopView extends StatefulWidget {
  const AddCityDesktopView({super.key});

  @override
  State<AddCityDesktopView> createState() => _AddCityDesktopViewState();
}

class _AddCityDesktopViewState extends State<AddCityDesktopView> {
  late final provider = Provider.of<AddCityViewModel>(context);
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 10,),
                InkWell(onTap: (){
                  Provider.of<GeneralViewModel>(context,listen: false).updateSelectedIndex(index: CITY_INDEX);
                },child: const Text('مدن التوصيل',style: TextStyle(color: AppColors.mainColor,fontSize: 13),)),
                const SizedBox(width: 10,),
                const Text('/',style: TextStyle(color: AppColors.mainColor,fontSize: 13),),
                const SizedBox(width: 10,),
                const Text('إضافة مدينة توصيل',style: TextStyle(color: Colors.grey,fontSize: 13),),
              ],
            ),
            _buildDivider(),
            const Text(
              ...
              children: [
                InkWell(
                  onTap: provider.loading? (){} : () async {
                    await Provider.of<AddCityViewModel>(context,
                          listen: false)
                          .addCity(
                        context
                          );},
