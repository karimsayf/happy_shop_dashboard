...
class EmployeesViewModel with ChangeNotifier {
  ...
  Future getEmployeesHome(BuildContext context, String page) async {
    ...
    await Provider.of<ApiServicesViewModel>(context, listen: false).getData(
        apiUrl: "$baseUrl/api/v1/employees?page=$page&size=10",
        headers: {
          'Authorization':
      ...
      if (getCustomersResponse["status"] == "success") {
        clearEmployees();
        clearData();
        employees = getCustomersResponse["data"]["employees"]
            .map<EmployeeModel>((e) => EmployeeModel.fromJason(e))
            .toList();
  ...
  Future searchEmployees(
    ...
    await Provider.of<ApiServicesViewModel>(context, listen: false).getData(
      ...
      if (searchCustomersResponse["status"] == "success") {
        employees = searchCustomersResponse["data"]["employees"]
            .map<EmployeeModel>((e) => EmployeeModel.fromJason(e))
            .toList();
        if (employees.isEmpty) {
          employeesEmpty = true;
        ...
        if (searchCustomersResponse["data"] is Map &&
          ...
          showCustomToast(context, searchCustomersResponse["data"]["message"],
              "assets/icons/alert-circle.webp", AppColors.c999);
        } else {
  ...
  Future deleteEmployee(BuildContext context, String employeeId) async {
    ...
    await Provider.of<ApiServicesViewModel>(context, listen: false).deleteData(
      ...
      if (deleteSubsectionsResponse["status"] == "success") {
        ...
        showCustomToast(context, "تم حذف الموظف بنجاح",
  ...
  Future updateEmployeeStatus(
      BuildContext context, String employeeId, String status) async {
    ...
    await Provider.of<ApiServicesViewModel>(context, listen: false).updateData(
      ...
      if (deleteSubsectionsResponse["status"] == "success") {
        ...
        Navigator.pop(context);
        if (status == "ACTIVE") {
          showCustomToast(context, "تم الغاء تعليق الموظف بنجاح",
              "assets/icons/check_c.webp", AppColors.c368);
        } else {
          showCustomToast(context, "تم تعليق الموظف بنجاح",
              "assets/icons/check_c.webp", AppColors.c368);
        }
        getEmployees(context, "0", true);
      } else {
